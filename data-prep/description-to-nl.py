...
def rewrite_description(description, few_shot_examples, model_name):
    ...
    prompt = (
        f"{few_shot_examples}\n"
        f"Rewrite the following description into descriptive format. Respond with only the description, do not write 'Here is the rewritten output' etc. Remove mediums such as painting, cartoon:\n"
        f"Input: {description}\n"
        f"Output:"
    )
...
def main():
    ...
    parser.add_argument("--model_name", type=str, help="Name of the LLM model to use.", default="llama3.1:8b-instruct-q5_1")
    parser.add_argument("--examples_file", type=str, help="File containing few-shot examples.", default="prompts/description-to-nl-examples.txt")
